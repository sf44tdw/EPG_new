...
/**
 * IP放送は考慮しない。 セクションのテーブル識別値(テーブルID)とそのテーブルの最大長の組み合わせ。 テーブルIDは1つ以上必要。
 * 2箇所以上で同じテーブルIDを指定することは禁止。 値はARIB STD-B10 第１部 P13より抜粋
 */
public enum TABLE_ID {
     ...
     */
    SDT_THIS_STREAM("サービス記述テーブル（自ストリーム）", MAX_SECTION_LENGTH.BYTE_1021, ServiceDescriptionTableBody.class, 0x42),
    /**
     * サービス記述テーブルSDT（Service Description Table）<br>
     * 編成チャンネルの名称、放送事業者の名称など、編成チャンネルに関する情報を伝送する。<br>
     * 最大セクション長は1021バイト。<br>
     ...
     * また、自トランスポートストリーム以外のトランスポートストリームのSDTセクションのテーブル識別値は0x46とする。<br>
     */
    SDT_OTHER_STREAM("サービス記述テーブル（他ストリーム）", MAX_SECTION_LENGTH.BYTE_1021, ServiceDescriptionTableBody.class, 0x46),
    /**
     * EIT(自局8日以内)
     */
